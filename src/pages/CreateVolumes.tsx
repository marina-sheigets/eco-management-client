...
import {
	...
	createVolumesAction,
	clearVolumesStatusAction,
} from '../redux/api/ApiActions';
import { getListOfEnterprises, getListOfDepartments, getVolumesStatus } from '../redux/selectors';
...
function CreateVolumes() {
	...
	const status = useSelector(getVolumesStatus);
	...
	const [typeOfVolumes, setTypeOfVolumes] = useState('');
	...
	const handleCreateVolumes = () => {
		dispatch(
			createVolumesAction.request({
				enterprise: enterpriseName,
				department: departmentName,
				type: typeOfVolumes,
				volumes: yearsInterval,
			})
		);
	};

	const handleCloseStatusMessage = () => {
		dispatch(clearVolumesStatusAction.request());
	};
			...
			<Wrapper>
				...
				<TextField
					...
					onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
						setTypeOfVolumes(e.target.value)
					}
					select
					color='primary'
					size='small'
				...
				{!isEditingMode ? (
					...
					<>
						<Typography>Empty cells will automatically equal 0.</Typography>
						<YearsRange>
